...
class Show extends Component
{
    ...
    public function mount(?string $server_uuid = null)
    {
        ...
        $server = Server::query()->where('team_id', $teamId)->where('uuid', $serverUuid)->first();
        if (! $server) {
    ...
    {
        $this->variables = $this->formatEnvironmentVariables($this->server->environment_variables()->whereNotIn('key', ['COOLIFY_SERVER_UUID', 'COOLIFY_SERVER_NAME'])->get()->sortBy('key'));
    }
