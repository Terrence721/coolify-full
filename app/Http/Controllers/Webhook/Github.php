...
class Github extends Controller
{
        ...
        try {
            ...
            foreach ($applicationsByServer as $serverId => $serverApplications) {
                foreach ($serverApplications as $application) {
                    ...
                    $isFunctional = data_get($application, 'destination.server')?->isFunctional();
                    if (! $isFunctional) {
    ...
    public function normal(Request $request): mixed
    {
        try {
            ...
            $applications = Application::where('repository_project_id', $id)
                ...
                ->whereHasMorph('source', [GithubApp::class], fn ($query) => $query->where('is_public', false));
            if ($x_github_event === 'push') {
    ...
    {
        $privateKey = $github_app->privateKey()->first();
        if (blank($github_app->app_id) || blank($privateKey?->private_key)) {
            return false;
        }
