...
class ServersController extends Controller
{
    ...
    public function servers(Request $request): JsonResponse
    {
        ...
        $servers = ModelsServer::whereTeamId($teamId)->select('id', 'name', 'uuid', 'ip', 'user', 'port', 'description')->get()->load(['settings'])->map(function (ModelsServer $server) {
            $settings = $server->settings()->first();
            $server['is_reachable'] = (bool) data_get($settings, 'is_reachable', false);
            $server['is_usable'] = (bool) data_get($settings, 'is_usable', false);

            return $server;
        });
        $servers = $servers->map(function (ModelsServer $server) {
