...
class Emails extends Command
{
        ...
        switch ($type) {
            ...
            case 'application-deployment-success-daily':
                ...
                foreach ($applications as $application) {
                    ...
                    $this->mail = (new DeploymentSuccess($application, 'test'))->toMail($this->resolveNotifiable());
                    $this->sendEmail();
                }
                break;
            ...
            case 'application-deployment-failed':
                ...
                $this->mail = (new DeploymentFailed($application, 'test'))->toMail($this->resolveNotifiable());
                $this->sendEmail();
                $this->mail = (new DeploymentFailed($application, 'test', $preview))->toMail($this->resolveNotifiable());
                $this->sendEmail();
                break;
    ...
    private function resolveNotifiable(): SendsEmail
    {
        $email = $this->email ?? 'test@example.com';

        return new class($email) implements SendsEmail
        {
