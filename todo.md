...
| Full detail | Scrum issue #32 — the single source of truth for this initiative: tracks not just the 24 suites done so far (setup, per-suite rationale, verification) but also the full remaining backlog (122 of 142 Components/Pages still untested, listed and checked off individually as each gets a suite). This table only shows current totals; #32 is where "what's actually left" lives. |
...
| # | Item | Status |
|---|---|---|
| 1 | Manual SSH-touching smoke-test checklist | In progress — 11 tracked sub-issues (#5), 3 of 11 done (including #21, now 28/28). See "Migration follow-up" and `docs/smoketest.md` |
...
| Item | Status |
|---|---|
| Every SSH-touching action converted so far has an untested happy-path gap (verified only via safe/validation-rejection paths in Pest) | In progress — manual QA checklist in `docs/smoketest.md`, split across 11 tracked sub-issues (issue #5), 3 of 11 done (including #21, now 28/28) |
