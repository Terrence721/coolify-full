 ...
 * say the same thing.
 */
class ValidateAndInstallServerJobTest extends TestCase
{
    use RefreshDatabase;
    ...
    public function an_outdated_but_installed_docker_engine_reports_the_real_problem_not_missing_installation(): void
    {
        ...
        $this->assertNotNull($validationLogsCall, 'expected an update() call setting validation_logs');
        $this->assertStringContainsString('too old', $validationLogsCall['validation_logs']);
        $majorRequiredVersion = str(config('constants.docker.minimum_required_version'))->before('.')->value();
        $this->assertStringContainsString($majorRequiredVersion, $validationLogsCall['validation_logs']);
        $this->assertStringNotContainsString('is not installed', $validationLogsCall['validation_logs']);
    }
}
