...
it('hands off to Server\\Show as a plain redirect for Inertia requests, now that Server\\Show is an Inertia page', function () {
    ...
    // an Inertia client-side visit — no more Inertia::location()/409 hand-off needed, that was
    // only required back when server.show was still a plain Livewire/Blade full-page route.
    $user = User::factory()->create();
    $team = Team::factory()->create();
    $team->members()->attach($user, ['role' => 'admin']);
    $key = PrivateKey::create([
