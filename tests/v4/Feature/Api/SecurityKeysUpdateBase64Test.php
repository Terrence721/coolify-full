<?php

declare(strict_types=1);

use App\Models\Team;
use App\Models\User;
use Illuminate\Foundation\Testing\RefreshDatabase;
use Tests\Support\InteractsWithApiV1;

uses(RefreshDatabase::class, InteractsWithApiV1::class);

beforeEach(function () {
    $this->apiEnable();
});

// Throwaway RSA key pair generated solely for this test fixture, not a real credential.
const SECURITY_KEYS_UPDATE_BASE64_TEST_KEY = <<<'KEY'
-----BEGIN RSA PRIVATE KEY-----

-----END RSA PRIVATE KEY-----
KEY;

it('accepts a base64-encoded private_key on update, matching create()', function () {
    $team = Team::factory()->create();
    $user = User::factory()->create();
    $token = $this->apiToken($user, $team, ['write'], role: 'admin');

    $base64Key = base64_encode(SECURITY_KEYS_UPDATE_BASE64_TEST_KEY);

    $createResponse = $this->withHeaders($this->apiHeaders($token))->postJson('/api/v1/security/keys', [
        'name' => 'original-name',
        'private_key' => $base64Key,
    ]);
    $createResponse->assertCreated();
    $uuid = $createResponse->json('uuid');

    // Resending the same base64 form the client used to create it - just renaming, not
    // changing the key material at all.
    $response = $this->withHeaders($this->apiHeaders($token))->patchJson("/api/v1/security/keys/{$uuid}", [
        'name' => 'renamed',
        'private_key' => $base64Key,
    ]);

    $response->assertCreated();
});
