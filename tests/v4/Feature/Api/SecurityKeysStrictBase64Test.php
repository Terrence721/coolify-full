<?php

declare(strict_types=1);

use App\Models\Team;
use App\Models\User;
use Illuminate\Foundation\Testing\RefreshDatabase;
use Tests\Support\InteractsWithApiV1;

uses(RefreshDatabase::class, InteractsWithApiV1::class);

beforeEach(function () {
    $this->apiEnable();
});

// Throwaway RSA key pair generated solely for this test fixture, not a real credential.
const SECURITY_KEYS_STRICT_BASE64_TEST_KEY = <<<'KEY'
-----BEGIN RSA PRIVATE KEY-----

-----END RSA PRIVATE KEY-----
KEY;

it('rejects a non-base64 private_key on create with a clean validation error, not a garbled key', function () {
    $team = Team::factory()->create();
    $user = User::factory()->create();
    $token = $this->apiToken($user, $team, ['write'], role: 'admin');

    $response = $this->withHeaders($this->apiHeaders($token))->postJson('/api/v1/security/keys', [
        'name' => 'bad-key',
        'private_key' => 'this is not base64 or PEM!!!',
    ]);

    $response->assertStatus(422);
    $response->assertJsonPath('errors.private_key', 'The private_key should be a valid PEM string or base64 encoded.');
});

it('rejects a non-base64 private_key on update with a clean validation error', function () {
    $team = Team::factory()->create();
    $user = User::factory()->create();
    $token = $this->apiToken($user, $team, ['write'], role: 'admin');

    $createResponse = $this->withHeaders($this->apiHeaders($token))->postJson('/api/v1/security/keys', [
        'name' => 'good-key',
        'private_key' => SECURITY_KEYS_STRICT_BASE64_TEST_KEY,
    ]);
    $uuid = $createResponse->json('uuid');

    $response = $this->withHeaders($this->apiHeaders($token))->patchJson("/api/v1/security/keys/{$uuid}", [
        'name' => 'renamed',
        'private_key' => 'this is not base64 or PEM!!!',
    ]);

    $response->assertStatus(422);
    $response->assertJsonPath('errors.private_key', 'The private_key should be a valid PEM string or base64 encoded.');
});
