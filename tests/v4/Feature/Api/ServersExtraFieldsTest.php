<?php

declare(strict_types=1);

use App\Models\PrivateKey;
use App\Models\Server;
use App\Models\Team;
use App\Models\User;
use Illuminate\Foundation\Testing\RefreshDatabase;
use Tests\Support\InteractsWithApiV1;

uses(RefreshDatabase::class, InteractsWithApiV1::class);

beforeEach(function () {
    $this->apiEnable();
});

// Throwaway RSA key pair generated solely for this test fixture, not a real credential.
const SERVERS_EXTRA_FIELDS_TEST_KEY = <<<'KEY'
-----BEGIN RSA PRIVATE KEY-----

-----END RSA PRIVATE KEY-----
KEY;

it('rejects an unexpected field on create', function () {
    $team = Team::factory()->create();
    $key = PrivateKey::create([
        'team_id' => $team->id,
        'name' => 'server-key',
        'private_key' => SERVERS_EXTRA_FIELDS_TEST_KEY,
    ]);
    $user = User::factory()->create();
    $token = $this->apiToken($user, $team, ['write'], role: 'admin');

    $response = $this->withHeaders($this->apiHeaders($token))->postJson('/api/v1/servers', [
        'name' => 'my-server',
        'ip' => '203.0.113.10',
        'private_key_uuid' => $key->uuid,
        'unexpected_field' => 'nope',
    ]);

    $response->assertStatus(422);
    $response->assertJsonValidationErrors('unexpected_field');
});

it('still creates a server with only allowed fields', function () {
    $team = Team::factory()->create();
    $key = PrivateKey::create([
        'team_id' => $team->id,
        'name' => 'server-key',
        'private_key' => SERVERS_EXTRA_FIELDS_TEST_KEY,
    ]);
    $user = User::factory()->create();
    $token = $this->apiToken($user, $team, ['write'], role: 'admin');

    $response = $this->withHeaders($this->apiHeaders($token))->postJson('/api/v1/servers', [
        'name' => 'my-server',
        'ip' => '203.0.113.11',
        'private_key_uuid' => $key->uuid,
    ]);

    $response->assertCreated();
});

it('rejects an unexpected field on update', function () {
    $team = Team::factory()->create();
    $key = PrivateKey::create([
        'team_id' => $team->id,
        'name' => 'server-key',
        'private_key' => SERVERS_EXTRA_FIELDS_TEST_KEY,
    ]);
    $server = Server::factory()->create([
        'team_id' => $team->id,
        'private_key_id' => $key->id,
    ]);
    $user = User::factory()->create();
    $token = $this->apiToken($user, $team, ['write'], role: 'admin');

    $response = $this->withHeaders($this->apiHeaders($token))->patchJson("/api/v1/servers/{$server->uuid}", [
        'name' => 'renamed',
        'unexpected_field' => 'nope',
    ]);

    $response->assertStatus(422);
    $response->assertJsonValidationErrors('unexpected_field');
});
