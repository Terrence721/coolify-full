...
**Update (2026-07-17):** the Livewire file referenced above no longer exists — the Livewire→React migration completed 2026-07-14 and `app/Livewire/`/`resources/views/livewire/` were deleted once empty. Re-verified against the React replacement, `resources/js/Components/ResourceTabs.jsx`: every AC still holds — `<select>` dropdowns for server/destination (Clone) and project/environment (Move), the action button only renders once a selection is made, and a `canUpdate` check gates the whole section with the same "you don't have permission" message. Status stays Done; just correcting the file reference.
