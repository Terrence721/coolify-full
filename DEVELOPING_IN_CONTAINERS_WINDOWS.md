...
2. **`./scripts/dev-up.sh` (run this after logging back in following a reboot, or a Docker Desktop upgrade)** — host-side, so it's immune to the file-mount OCI-creation race that killed the container-native attempt. Detects and fixes both failure shapes: retries `docker restart coolify` until its directory mount is populated, and separately retries `docker restart` on `coolify-realtime`/`coolify-autoheal`/`coolify-testing-host` until each reports `running`. In real testing the file-mount race took up to ~10 minutes post-boot to clear on its own, so the script retries with real persistence (up to 3 minutes per service) rather than giving up after a few seconds.
...
**Correction, 2026-07-22 (a Docker Desktop *upgrade*, not just a reboot, surfaces a third variant):** found via the `/settings/scheduled-jobs` smoke test — the "Skipped Jobs" tab showed `docker cleanup` skipped every single scheduler run, always with reason "Server not functional," for the dev server (`production-01`, backed by the `coolify-testing-host` container). `coolify-testing-host` had come up with container status `running` (the earlier fix's `docker restart`-until-`running` check passed), but `docker exec coolify-testing-host docker version` showed a real `Client` block and a **null `Server` block** — "Cannot connect to the Docker daemon at unix:///var/run/docker.sock" — its `docker.sock` mount was a stale/broken socket reference from before the upgrade, even though the container itself was healthy and running the whole time. This silently broke `ServerConnectionCheckJob`'s Docker-availability check (`app/Models/Server.php`'s `isFunctional()` requires both `is_reachable` and `is_usable`), which cascades into `ScheduledJobManager` skipping every docker-cleanup run against this server. A plain `docker restart coolify-testing-host` fixed it immediately (confirmed via `docker version` returning a real `Server` block afterward), and re-dispatching `ServerConnectionCheckJob` synced `is_usable` back to `true`. `dev-up.sh` updated to add a functional check specifically for `coolify-testing-host` — verifying `docker version`'s `Server` block is non-null, not just that the container's status is `running` — since "running" alone isn't sufficient evidence this mount actually reconnected.
