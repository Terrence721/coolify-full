...
- 55 of 84 full-page Livewire components converted to Inertia + React.
- Easy and Medium buckets: 100% done (25/25).
- Hard bucket: 28 of 59 done, including the shared `Server` navbar/sidebar chrome and eight non-`Server`-scoped Hard-bucket pages (`Security\PrivateKey\Index`, `Destination\Index`, `Project\Show`, `Project\Edit`, `Storage\Index`, `Project\Index`, `Storage\Show`, `Storage\Resources`). The whole `Storage\*` Livewire area is now fully retired (except the shared `Storage\Create` modal, still used by `GlobalSearch`).

## Still to do

**Migration**
- 31 Hard-bucket pages remain on Livewire, including 5 of 21 `Server\Navbar`-dependent pages (Terminal command, `Server\Show`, plus Dynamic Configurations/Logs within Proxy and Logs within Sentinel). `Server\Show` and Terminal both need real design work before conversion (embedded Livewire island / WebSocket bridge, respectively) — see Section 66 of the migration doc.
