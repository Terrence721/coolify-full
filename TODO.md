...
- 46 of 84 full-page Livewire components converted to Inertia + React.
- Easy and Medium buckets: 100% done (25/25).
- Hard bucket: 19 of 59 done, including the shared `Server` navbar/sidebar chrome that the remaining Server-scoped pages build on.

## Still to do

**Migration**
- 40 Hard-bucket pages remain on Livewire, including 6 of 21 `Server\Navbar`-dependent pages (Sentinel, Terminal command, `Server\Show`, plus Dynamic Configurations/Logs within the Proxy area). The next one (`Server\Sentinel\Show`) needs a new sidebar-variant design pass before conversion — see Section 52 of the migration doc.
