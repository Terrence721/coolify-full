...
/**
 * Simplified port of livewire/project/application/heading.blade.php, shared by
 * Project/Shared/Logs (Phase 47) and Project/Application/Configuration (Phase 64) — its
 * second consumer, promoted here from a page-scoped component the same way
 * DatabaseHeading/ServiceHeading were. Known v1 gaps, same "documented, not silently
 * dropped" pattern as those two: the resource breadcrumb trail (x-resources.breadcrumbs),
 * the domain-link chips (x-applications.links), the "advanced" button group
 * (x-applications.advanced), the restart-count warning icon on the Logs link, the mobile
 * dropdown Actions menu, and the Stop confirmation's docker_cleanup checkbox (hardcoded
 * true here, matching DatabaseHeading/ServiceHeading's precedent) are not ported.
 */
export default function ApplicationHeading({ application, heading, parameters, urls }) {
    const { props } = usePage();
    ...
    const isSwarm = application.isSwarm ?? false;
    const isDockerCompose = application.buildPack === 'dockercompose';
    const needsComposeFile = isDockerCompose && !application.hasDockerCompose;
        ...
        <nav className="pb-6">
            ...
            <div className="navbar-main">
                <nav className="flex min-h-10 w-full flex-nowrap items-center gap-6 overflow-x-scroll whitespace-nowrap">
                    ...
                    <a className="shrink-0" href={route_('project.application.deployment.index', parameters)}>
                        Deployments
                    </a>
                    <a className="shrink-0" href={route_('project.application.logs', parameters)}>
                        Logs
                    </a>
                    {!isSwarm && canAccessTerminal && (
                        <a className="shrink-0" href={route_('project.application.command', parameters)}>
                            Terminal
                        </a>
                    )}
                </nav>
                    ...
                    <div className="flex flex-wrap gap-2 items-center">
                        {!isExited ? (
                            <>
                                ...
                                {!isDockerCompose &&
                                    (isSwarm ? (
                                        <button type="button" title="Redeploy Swarm Service (rolling update)" onClick={() => deploy(false)}>
                                            Update Service
                                        </button>
                                    ) : (
...
function route_(name, parameters) {
    ...
    switch (name) {
        ...
        case 'project.application.logs':
            return `${base}/logs`;
        case 'project.application.command':
