...
**high · Reliability + information disclosure — every method in the file** — Fixed via [PR #215](https://github.com/Terrence721/coolify-full/pull/215) ([`00387440b`](https://github.com/Terrence721/coolify-full/commit/00387440bacfdcde8558927f20def96887f54c3d))

Found via the same pass above. Three methods (`create_github_app`, `load_repositories`, `load_branches`) delegated unhandled exceptions to `handleError()` — a helper built for Livewire's error-dispatch mechanism. Outside a Livewire context, it either returns a bare string for two specific exception types (which Laravel wraps in a 200 OK, so a real failure reports success) or throws a fresh `Exception` for anything else, propagating fully uncaught to Laravel's default debug-mode exception handler — leaking a full stack trace and internal file paths in the JSON response. No other `Api` controller in this codebase calls `handleError()`; they all build their own `response()->json(['message' => ...], code)`. The other three methods (`update_github_app`, `delete_github_app`, `list_github_apps`) had no catch-all at all, unlike every sibling method in this file — `list_github_apps()` had no try/catch of any kind. Fixed by bringing all 6 methods in line with the rest of the codebase's convention.

---

### [`GithubApp.php`](https://github.com/Terrence721/coolify-full/blob/main/app/Models/GithubApp.php)
...
### [`ScheduledTasksController.php`, `DeployController.php`](https://github.com/Terrence721/coolify-full/blob/main/app/Http/Controllers/Api/ScheduledTasksController.php)
...
### [`Api/TeamController.php`](https://github.com/Terrence721/coolify-full/blob/main/app/Http/Controllers/Api/TeamController.php)

**low · Reuse/simplification, no behavior change** — Fixed via [PR #248](https://github.com/Terrence721/coolify-full/pull/248) ([`f4604e080`](https://github.com/Terrence721/coolify-full/commit/f4604e080))

The two lower-priority findings from the same pass above. `team_by_id()` called `serializeApiResponse()` twice on the same team — once inside `removeSensitiveData()`, once again explicitly; harmless today since the transform is idempotent, but relied on that being true. Fixed to match `current_team()`'s already-correct single-call pattern. Separately, the "validate token → validate user → look up team scoped to the caller's membership" block was copy-pasted nearly verbatim across all 5 of this controller's actions — extracted into `resolveAuthenticatedUserId()` and `resolveTeamForUser()`. **Caught mid-refactor**: `resolveTeamForUser()` strictly type-hints `int $teamId`, and `current_team()`/`current_team_members()` pass `getTeamIdFromToken()`'s return value as that target — which finding #114's own PR #236 precedent established can genuinely be a numeric *string* in production. Passing it uncast would have reintroduced that exact `TypeError` under this file's `declare(strict_types=1)`; added an explicit `(int)` cast at both call sites before that ever shipped. Also found while verifying: `teams()`, `current_team()`, and `current_team_members()` had zero test coverage anywhere in the suite — added basic coverage for all three since the refactor touches them directly.

---

### [`ManagesApiResourceStorages.php`](https://github.com/Terrence721/coolify-full/commit/b4a8e1d6ce1c6e6fc8f7db7b5eec981fa0d4239e#commitcomment-198729299)

**high · Correctness/security — API clients could edit or delete docker-compose-managed volumes the UI treats as immutable** — Fixed via [PR #249](https://github.com/Terrence721/coolify-full/pull/249) ([`ee852f362`](https://github.com/Terrence721/coolify-full/commit/ee852f362))

Found via an independent `/code-review` pass on this trait — 473 lines, shared by ApplicationsController/DatabasesController/ServicesController, never previously reviewed, written during this fork's own Phase 5 refactor (not inherited). Independently verified by two background verification passes before fixing. `LocalPersistentVolume::isDockerComposeResource()` has its own explicit N+1-avoidance guard: `if (! $this->relationLoaded('resource')) { return false; }`, and `shouldBeReadOnlyInUI()` depends on it. `findApiStorageByLookup()`/`findApiStorageByUuid()` — the single shared lookup every update/delete storage endpoint in all 3 controllers goes through — located storages via plain lazy relation access, never setting the inverse `resource` relation back on the found model. So for a docker-compose-built `Application`, the read-only check silently evaluated to `false` through the API even though the same volume is correctly immutable in the UI — letting an API client PATCH or DELETE a compose-managed volume it shouldn't be able to touch. (Service-owned storages were never affected — `isServiceResource()` short-circuits before the relation check.) Fixed at the source: both lookup methods now call `$storage?->setRelation('resource', $resource)` before returning, using the resource already in hand — zero extra queries. TDD-proved: reverted the fix, confirmed both new reject-tests fail with the exact real symptom (200 instead of 422, the mutation actually went through), confirmed clean after restoring. Four more findings from the same pass are queued separately: an API storage-content update that never syncs to the actual remote server file, a `mount_path` validation gap between create and update, a directory/content invariant the update path doesn't enforce, and an N+1 query pattern.

---

### [`ManagesApiResourceStorages.php`](https://github.com/Terrence721/coolify-full/commit/b4a8e1d6ce1c6e6fc8f7db7b5eec981fa0d4239e#commitcomment-198730634)

**high · Correctness — an API-side file update silently never wrote the file to the actual server** — Fixed via [PR #250](https://github.com/Terrence721/coolify-full/pull/250) ([`841f579f1`](https://github.com/Terrence721/coolify-full/commit/841f579f1))

Second finding from the same pass above. `applyApiStorageUpdate()` persisted a file storage's new `content`/`mount_path` to the DB and returned 200, but `LocalFileVolume::booted()` only dispatches `ServerStorageSaveJob` on the `created` event, not `updated` — the client's response said success, but the actual file on the remote server kept its old contents until the resource's next full deployment. The web UI's `ManagesResourceStorages::updateStorageFile()` already calls `saveStorageOnServer()` explicitly after saving; this API path never got the same treatment. Fixed by calling it for `LocalFileVolume` instances, with the same revert-on-failure pattern the web version uses. **Found a second, latent bug while building that revert path**: the web pattern being mirrored — `$original = $file->getOriginal(); ...; $file->setRawAttributes($original);` — mismatches for `LocalFileVolume::content`, which is `encrypted`-cast. `getOriginal()` returns the *decrypted* value; `setRawAttributes()` expects the still-encrypted raw form. Pairing them silently stores plaintext where ciphertext belongs, corrupting the row on the next read — confirmed live via tinker, and caught by this PR's own new test (a `DecryptException`) before it ever shipped. Fixed by using `getRawOriginal()` instead. The web `updateStorageFile()` this pattern was copied from has zero test coverage of its own failure path and likely carries the identical latent bug — flagged, not fixed here (different file, out of scope for this PR).

---

### [`ManagesApiResourceStorages.php`](https://github.com/Terrence721/coolify-full/commit/b4a8e1d6ce1c6e6fc8f7db7b5eec981fa0d4239e#commitcomment-198731767)

**medium · Correctness — update's mount_path validation was weaker than create's for the identical field** — Fixed via [PR #252](https://github.com/Terrence721/coolify-full/pull/252) ([`fc23cd64b`](https://github.com/Terrence721/coolify-full/commit/fc23cd64b))

Third finding from the same pass above. `applyApiStorageCreate()`'s file-type branch normalizes `mount_path` (trim + forced leading slash) and validates it via `validateShellSafePath()` before storing it. `applyApiStorageUpdate()`'s file-type branch set `mount_path` directly from request input, skipping both — a client PATCHing a relative path, or one containing shell metacharacters, could persist a value `create()` would have rejected or normalized outright for the identical field. Fixed by applying the same normalize + `validateShellSafePath()` treatment to update. (Persistent-type `mount_path` is unaffected — create() itself never validates it either, so there was no established behavior to mirror there; a separate, broader question not part of this finding.) TDD-proved: reverted just the controller change (kept the new tests), confirmed both fail with the exact real symptom (no normalization; a shell-unsafe value silently accepted, 200 not 500), confirmed clean after restoring.

---

### [`ManagesApiResourceStorages.php`](https://github.com/Terrence721/coolify-full/commit/b4a8e1d6ce1c6e6fc8f7db7b5eec981fa0d4239e#commitcomment-198733494)
...
### [`ProjectApplicationConfigurationController.php`](https://github.com/Terrence721/coolify-full/commit/8317c1501dc2715c5866f3f071ca50f7b32e49a0#commitcomment-199283011)

**high · Reliability — an uncaught crash on a malformed fqdn, in two files at once — a regression from these ports, not inherited from upstream** — Fixed via [PR #260](https://github.com/Terrence721/coolify-full/pull/260) ([`e702acc31`](https://github.com/Terrence721/coolify-full/commit/e702acc31))

Found via an independent `/code-review` pass on this file — 2,593 lines, the largest controller in the app, never previously reviewed. `applyNormalizedFqdn()` calls `Url::fromString($domain, ['http', 'https'])` with no try/catch; `generalValidationRules()`'s `'fqdn' => 'nullable'` has no format check that would catch a malformed domain first (a bad port, a non-http(s) scheme) before it reaches that call. Checked against pristine upstream `coollabsio/coolify` v4.1.2: the original Livewire `App\Livewire\Project\Application\General::submit()` has the identical unguarded call, but wraps its **entire method body** in `try { ... } catch (\Throwable $e) { return handleError($e, $this); }` — the React port (`8317c1501`, Phase 69) split this into a standalone helper and never re-wrapped the caller, losing upstream's own safety net. A second, sibling instance of the exact same regression was found and fixed in the same PR: `ProjectServiceResourceController::updateApplication()`'s `normalizeFqdn()` call (Phase 49, `2271b225c`) has the identical gap for the Service-side equivalent — `ProjectServiceConfigurationController::updateChildDomain()` already had the correct try/catch for this same shared trait method, this call site just never got it. Fixed both with the same `try`/`catch (\Throwable $e)` shape. TDD-proved: reverting both fixes together reproduces the real symptom in both files, then restored.

---

### [`ProjectApplicationConfigurationController.php`](https://github.com/Terrence721/coolify-full/commit/abb1fad2879eb76e09e8ec76c89e3c2d4e6f852f#commitcomment-199283838)

**medium · Reliability — stopping an app on an additional server never refreshed that server's own status — inherited verbatim from upstream** — Fixed via [PR #261](https://github.com/Terrence721/coolify-full/pull/261) ([`71c559e17`](https://github.com/Terrence721/coolify-full/commit/71c559e17))

`serversStop()` stops containers on the specific server the request targeted, then unconditionally refreshes container status for `$application->destination->server` — the primary server — instead of the server that was just stopped. Checked against pristine upstream: `App\Livewire\Project\Shared\Destination::stop($serverId)` calls a shared `refreshServers()` helper with the identical bug, always using the primary destination's server regardless of which server `$serverId` actually targeted. For a multi-server application, this left the additional server's displayed status stale until something else happened to refresh it. Fixed by passing the correct server. New test mocks `GetContainersStatus` via container binding (matching this codebase's established `AsAction`-mocking pattern) and asserts `handle()` receives the right server. TDD-proved against the pre-fix code.

---

### [`ProjectApplicationConfigurationController.php`](https://github.com/Terrence721/coolify-full/commit/abb1fad2879eb76e09e8ec76c89e3c2d4e6f852f#commitcomment-199284499)

**medium · Correctness — the Rollback tab never identified the current image for a registry with an explicit port — inherited verbatim from upstream** — Fixed via [PR #262](https://github.com/Terrence721/coolify-full/pull/262) ([`ff16c164f`](https://github.com/Terrence721/coolify-full/commit/ff16c164f))

`rollbackLoadImages()` split the running image's full reference on `:` and took index 1 as the tag — correct for `nginx:latest` (2 parts) but wrong for `registry.example.com:5000/myapp:latest` (3 parts, where index 1 is `5000/myapp`, not the tag). The tag is always the *last* colon-separated segment; a registry port is the only other place a colon legitimately appears, and it always precedes the tag. Fixed by taking the last element instead of a fixed index, preserving the original `null` fallback for a tag-less reference. New test added a controller-scoped `instant_remote_process()` test fake (`tests/Support/Fakes/controllers_remote_process_overrides.php`) since this endpoint calls it directly rather than through an Action class — no such override existed yet for `App\Http\Controllers`. TDD-proved against the pre-fix code.

---

### [`ProjectApplicationConfigurationController.php`](https://github.com/Terrence721/coolify-full/commit/9f142c3c290cf7bda96754da7d47df5f8f87820b#commitcomment-199285454)
