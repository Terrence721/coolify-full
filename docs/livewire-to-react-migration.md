...
**As of Phase 79 (2026-07-14) — MIGRATION 100% COMPLETE.** 84 of 84 full-page Livewire components converted, and the last remaining chrome/navigation infrastructure (`layouts/app.blade.php`, `components/navbar.blade.php`, `GlobalSearch`, `LayoutPopups`, `DeploymentsIndicator`, `SwitchTeam`, `Server\Create`/`New\ByHetzner`/`New\ByIp`) is deleted. `app/Livewire/` and `resources/views/livewire/` are both confirmed completely empty (Phase 80, Section 147) — there is no remaining Hard-bucket count to track. This table previously drifted stale twice before (see the note appended to Section 65's writeup) and was left un-updated again through Phases 78–81; corrected here rather than left to drift a third time.
...
## 147. Post-completion audit — `app/Livewire/` is now completely empty

Prompted by the user raising the stakes explicitly: this codebase is being used as work-sample evidence for a job application, and the bar is "pristine, zero known issues" rather than "good enough." That's a real, stated reason to go back and audit rather than trust Section 146's own cascade as exhaustive.

It wasn't. A full sweep for every `<livewire:...>` tag anywhere in `resources/views/` (not just the ones already known to be entangled with `auth/verify-email.blade.php`) turned up two more genuine orphans immediately: `ByHetzner.blade.php` (deleted in Phase 146) had embedded `<livewire:security.cloud-provider-token-form>` and `<livewire:security.private-key.create>` — both already fully superseded by React (`Security/CloudTokens.jsx`+`Server/CloudProviderToken.jsx` for the token form; `PrivateKeyCreateModal.jsx` for the key-create modal, both confirmed via their own backend controllers already doing the identical `CloudProviderToken::create()`/private-key-store work), but the old Livewire versions were never deleted because until Phase 146 deleted `ByHetzner.blade.php`, they still had a live, technically-reachable consumer.

More significantly, the same sweep turned up a genuinely dangling reference: `resources/views/livewire/project/service/heading.blade.php` still contained `<livewire:activity-monitor>` — a tag pointing at `App\Livewire\ActivityMonitor`, a class Phase 78 had already deleted. This blade file itself turned out to be harmless in practice (its own parent, `App\Livewire\Project\Service\Heading`, had zero consumers of its own — a fact Phase 78's own writeup had already noted in passing without following through on deleting it), but it's exactly the kind of loose end a "pristine" bar doesn't tolerate: a dead class left in the tree, referencing another dead class, inside a blade file with zero real consumers. Deleted `Project\Service\Heading` and its now-also-orphaned nested `Project\Shared\ConfigurationChecker` (the heading's only remaining consumer) together.

**Having found that, the natural next question was whether this was the only such gap** — so the audit went further: every remaining `App\Livewire\*.php` file, not just the ones already implicated, checked against every possible reachability path (blade tags — already swept to zero — direct route bindings, and cross-file class references beyond blade templates). Found 8 more, each with zero references anywhere outside their own file: `MonacoEditor`, `Project\AddEmpty`, `Project\Database\BackupNow`, `Project\New\EmptyProject` (already known and explicitly left alone as dead code back in Phase 51 — "confirmed and left alone," never actually deleted), `Project\Shared\GetLogs`, `Project\Shared\UploadConfig`, `Storage\Create`, `Tags\Deployments`, and `Team\Create` (the latter two explicitly described as "done"/superseded back in Phase 76, but likewise never actually deleted — the React replacement shipping was recorded as closing the task, and the old file just never got swept up). All 9 deleted along with their blade views, except `MonacoEditor`'s: `components/forms/monaco-editor.blade.php` turned out to be a shared view also rendered by a genuinely live, unrelated anonymous Blade component (`<x-forms.monaco-editor>`, used by `components/forms/textarea.blade.php`'s `useMonacoEditor` prop) — confirmed via its own PHP class file not existing (Laravel resolves anonymous components purely from the view file), so only the Livewire PHP class was deleted, the view stayed.

One more stray was caught in the same pass: `resources/views/livewire/project/application/destination.blade.php`, a view file with no backing Livewire class at all and no `view()` call anywhere referencing it — a leftover fragment from whenever its original component was removed, never cleaned up.

**`app/Livewire/` and `resources/views/livewire/` are now both completely empty directories.** Every single Livewire component that ever existed in this application has either been converted to React or confirmed dead and removed.

Verification: PHPStan baseline regenerated again (1693 → 1611, net −82 more), full suite still 996 passed (unchanged — none of these had test coverage, confirming they were genuinely unreachable in practice, not just unreferenced in source), Pint clean, `yarn build` clean.

**Two follow-on questions surfaced by this audit** — both were decisions, not obvious cleanup, so they were put to the user rather than decided unilaterally: (1) `livewire/livewire` and `danharrin/livewire-rate-limiting` are still Composer dependencies, and `@livewireStyles`/`@livewireScripts` still load in `layouts/base.blade.php`/`layouts/simple.blade.php`, but with `app/Livewire/` now empty, nothing in the entire application uses either package anymore. (2) 21 Blade files still carry `wire:*` attributes left over from when they were shared between Livewire and Blade rendering paths. The user answered "remove/strip both" to both questions — carried out in Section 148 below.

## 148. Composer dependency removal, vestigial `wire:*` stripping, and a stale-docblock sweep that caught two real bugs

Both follow-on questions from Section 147 were authorized by the user and carried out in the same pass, along with a full sweep of comments/docblocks still claiming Livewire infrastructure exists.

**Composer and config.** `composer remove livewire/livewire danharrin/livewire-rate-limiting --no-interaction` — confirmed `vendor/livewire`/`vendor/danharrin` gone and `composer.json` clean (residual `composer.lock` mentions are just other packages' own `suggest` metadata). Deleted `config/livewire.php` (confirmed zero `config('livewire...')` call sites first).

**`layouts/base.blade.php`, `layouts/simple.blade.php`, `resources/js/app.js`.** Removed the `@livewireScripts` directive from `simple.blade.php`. Deleted the ~90-line `document.addEventListener('livewire:init', ...)` browser-event bridge from `base.blade.php` (`window.Livewire.on('reloadWindow'/'info'/'error'/'warning'/'success', ...)`) — confirmed dead since nothing can ever dispatch a Livewire browser event again, and confirmed `window.toast` (which the bridge routed *to*) is independently defined in `components/toast.blade.php`'s own `x-init`, not inside the block being removed. This also fixed a real, if minor, regression already latent in `copyToClipboard()`: it called `window.Livewire.dispatch('success', ...)`, which would have silently no-opped the moment `window.Livewire` stopped existing — rewritten to call `window.toast(...)` directly. `resources/js/app.js` (previously just a dead `livewire:navigated` FOUC-workaround listener) rewritten to a two-line comment explaining it's now just the Vite entrypoint for the remaining Blade-only guest/auth/error pages.

**`PrivateKey::generateNewKeyPair()`** ported off `DanHarrin\LivewireRateLimiting\WithRateLimiting` onto plain `Illuminate\Support\Facades\RateLimiter` (`tooManyAttempts()`/`hit()`/`availableIn()`), same 10-attempts-per-60-seconds policy, keyed on `request()->ip()`.

**50 dead Blade components + 12 dead `App\View\Components\*` classes deleted**, found via a purpose-built transitive-closure reachability script (walks every `<x-tag>` reference recursively from a confirmed-live entry-point set — all `auth/`, `invitation/`, `errors/`, `emails/` blade views plus `layouts/base.blade.php`/`layouts/simple.blade.php` — and reports every file under `resources/views/components/` never reached). First pass produced a false positive (`components/emails/{header,footer,layout}.blade.php` flagged dead) because Mailable classes render `emails.*` views through a separate `view('emails.xxx')` pathway the initial entry-point set didn't cover — caught before deleting, fixed by adding `emails/*.blade.php` as entry points. Only 11 components survived as genuinely live: `external-link`, `forms/button`, `forms/input`, `helper`, `highlighted`, `layout-simple`, `loading-on-button` (see below — this one didn't survive the *next* pass), `toast`, `emails/{header,footer,layout}`. One pre-existing broken class was found in the process: `App\View\Components\Services\Explanation` pointed at `services/explanation.blade.php`, a view that had never existed at all.

**Vestigial `wire:*` attributes.** Of the 21 files flagged in Section 147, all but two turned out to be part of the 50 already deleted as fully dead. The two survivors — `components/forms/button.blade.php` and `components/forms/input.blade.php`, both genuinely live (rendering every remaining Blade-only auth/error page) — still carried real `wire:*` markup:
- `input.blade.php`: `wire:model`, `wire:dirty.class`, `wire:loading.attr="disabled"` on both the password and plain-input branches, plus a `@success.window="type = 'password'"` Alpine listener that can now never fire (nothing dispatches a `success` window event anymore — only `toast-show` does). All removed; the `@error($modelBinding)` validation-error block was left untouched since `$modelBinding` also serves as the plain Laravel validation-error-bag key, not just the old `wire:model` binding name.
- `button.blade.php`: the `wire:click`/`wire:target`-detection loading-indicator branch (`<x-loading-on-button wire:target=... wire:loading.delay />`) could never fire either, since no live consumer of `<x-forms.button>` has ever passed a `wire:*` attribute (confirmed by grepping every real usage across `auth/`, `errors/`, `invitation/`). Removed, along with an already-dead `@isset($confirm)`/`@isset($confirmAction)` block that called a JS function (`toggleConfirmModal`) deleted along with `confirm-modal.blade.php` in this same round, and referenced Alpine's `$wire` magic variable, which no longer exists. Both branches' unused constructor props (`modalId`, `showLoadingIndicator`, plus a separately-confirmed-dead `modalId`) removed from `App\View\Components\Forms\Button`. `components/loading-on-button.blade.php` — reachable only from the now-deleted branch — was itself then orphaned and deleted.

**Stale-docblock sweep.** Grepped every remaining `livewire`/`wire:`/`App\Livewire\` mention across `app/` and `resources/js/` for present-tense claims that Livewire infrastructure still exists, now that `app/Livewire/` is confirmed completely empty. Found and rewrote ~10 stale docblocks (`AppLayout.jsx`, `GlobalSearchController.php`, `GlobalSearchModal.jsx`, `LayoutPopups.jsx`, `PrivateKeyCreateModal.jsx`, `SecurityPrivateKeyController.php`, `ActivityController.php`, `ProjectMetricsController.php`, `ManagesResourceEnvironmentVariables.php`, `Metrics.jsx`, `BoardingController.php`) that still said things like "the Livewire version stays alive unchanged for Boarding\Index/Server\Show" or "Application\Configuration is still Livewire" — both pages/routers had already been fully React for multiple phases by this point.

The same sweep caught **two real, live bugs**, not just stale comments — both silently swallowed by broad `catch (\Throwable $e)` blocks that logged to Ray and moved on rather than surfacing:
- `App\Traits\ClearsGlobalSearchCache` had `use App\Livewire\GlobalSearch;` and called `GlobalSearch::clearTeamCache($teamId)` in all three of its model-event hooks (`saving`/`created`/`deleted`) — a class that no longer existed, meaning global-search cache invalidation had been silently failing on every relevant model write since `App\Livewire\GlobalSearch` was deleted in Section 147, for every model using the trait (`Application`, `Server`, `Service`, `Project`, `Environment`). The `ClearGlobalSearchCache` console command had already been correctly repointed to `GlobalSearchService::clearTeamCache()` in Section 147's own writeup — this trait was the same class's other call site, missed at the time. Fixed by repointing it the same way.
- `App\Models\Server.php` had a dead `use App\Livewire\Server\Proxy;` import — a class that no longer existed, entirely unreferenced anywhere in the file body. PHPStan doesn't flag unused imports by default, which is how this survived every prior verification pass. Removed.

Both bugs were pre-existing before this session (not introduced by this cleanup) but were only surfaced by treating "the comment references a deleted class" as worth chasing down to its actual code, not just editing the prose. PHPStan's own baseline had 13 pre-registered `ignoreErrors` entries for the exact `Call to static method clearTeamCache() on an unknown class App\Livewire\GlobalSearch` error across the 8 standalone-database model files that mix in the trait — meaning this was a *known, previously-baselined-away* error, not a fresh regression, which is exactly the kind of "swept under the rug" issue this pass was meant to surface and fix rather than re-ignore.

Verification: Pint clean, PHPStan baseline regenerated (1611 → 1572, net −39 — the drop reflects both the dead-code deletions and the now-fixed `clearTeamCache` errors dropping out), full suite 996 passed (unchanged — the `clearTeamCache` bug had no test coverage either, consistent with it having silently failed for a while), `yarn build` clean.

## 149. The 388-element accessibility gap, closed for good

A follow-up to the "id or name" accessibility note flagged in Section 148 (originally raised 2026-07-10, re-scoped 2026-07-14): Chrome DevTools' Issues tab was correct that every `<input>`/`<select>`/`<textarea>` across `resources/js/` needed a real `id`/`name`, and a regex sweep had already sized it at 388 elements across 80 files. That pass deliberately stopped short of fixing it, for the reason it documented: a mechanical `id="foo"` injection would give every row of a `.map()`-rendered list (environment variables, shared variables, storage volumes, OAuth providers, etc.) the exact same static ID — a real duplicate-ID bug, worse than the original gap.

This pass did the actual fix, file by file, in six batches ordered by risk and size. For every file, the deciding question was: does this file render the field once per page, or once per row of a dynamic list? Single-instance forms (the large majority — settings pages, "add X" modals, resource edit forms) got flat, page-scoped IDs derived from the field's own semantic name (e.g. `id="settings-email-smtp-host"`). Files rendering fields inside a genuine `.map()` over user data — `EnvironmentVariablesTab.jsx`, `SharedVariablesManager.jsx`, `PreviewDeploymentsTab.jsx`, `StoragesTab.jsx` (volumes/files keyed by their own `id`), `SettingsOauth.jsx` (keyed by `provider`), `Team/Member/Index.jsx` (invitation links keyed by `invitation.id`), and others — got IDs built from each row's own stable identifier (e.g. `` `env-${env.id}-key` ``, `` `volume-${volume.id}-name` ``), the same fix the original note recommended but didn't attempt. A handful of `.map()`s turned out to iterate a small, hardcoded config array rather than user data (the `EVENT_GROUPS` notification-settings tables in `Notifications/{Email,Discord,Slack,Telegram,Pushover,Webhook}.jsx`, each field's own key already globally unique) — those got the field's own key as the ID directly, no extra scoping needed.

**A second pass caught a class of gap the first one's tooling couldn't see.** Several files (`ApplicationGeneralTab.jsx`, `DatabaseGeneralTab.jsx`, `ServiceStackTab.jsx`, `ScheduledTasksTab.jsx`, `DatabaseHealthcheckTab.jsx`, `ApplicationHealthcheckTab.jsx`, `AdvancedTab.jsx`, `StoragesTab.jsx`, `EnvironmentVariablesTab.jsx`) define a small local `Field`/`Checkbox`/`CredentialField` wrapper component around the real `<input>`, used dozens of times within the file. A plain regex sweep for literal `<input>` tags only ever sees that one definition — it has no way to know the wrapper is called 10, 20, sometimes 40 times, each one needing its own `id`. Fixing the wrapper's own internals (spreading `id`/`name` through via `{...props}` or an explicit prop) was necessary but not sufficient; every call site also needed an explicit `id` passed in, or every rendered instance shares the wrapper's default (nothing). Caught by writing a second, purpose-built script that walks each file looking for `<Field`/`<Checkbox`/`<CredentialField` blocks specifically and checking each one individually, rather than trusting the raw-tag count. `ServiceStackTab.jsx` alone had 3 `Field` calls the original per-file count of "1" (only counting the raw `<textarea>` elsewhere in the same file) had completely missed.

One early scripting mistake, caught before it shipped: an automated line-based insertion script initially placed the new `id`/`name` lines *after* a tag's closing `/>` for any single-line self-closing `<Field ... />` call, producing syntactically invalid JSX floating outside the tag. Caught immediately by `yarn build` failing to compile, not by manual review — a good reminder that the build step in this verification sequence isn't just a formality even for "just adding attributes" changes.

Verification: `yarn build` re-run after every batch (not just at the end) specifically because of the above near-miss; full Pest suite (996 passed, unchanged — this was a pure frontend change) confirmed nothing on the backend was affected; a final whole-codebase sweep (raw tags + wrapper-component call sites, the same two-part check used throughout) confirmed zero remaining elements, down from 388.

## 150. Hetzner Cloud server creation, rebuilt from scratch as a React flow

Phase 146 closed the loop on Hetzner Cloud server creation being unreachable from the UI: the last path (`GlobalSearch` → `Server\Create` → `ByHetzner`) was deleted along with the rest of the Livewire tree, with the explicit note that it would need to be "rebuilt as a new React flow from scratch" if ever wanted back. The user asked for exactly that.

**This turned out to be a UI-only rebuild, not a full-stack one.** The Hetzner Cloud API integration itself was never touched by the Livewire deletions — `App\Services\HetznerService` (locations/server-types/images/ssh-keys/create/get/power-on/delete, with 429 rate-limit handling and its own `tests/Unit/Services/HetznerServiceTest.php` coverage) and `App\Http\Controllers\Api\HetznerController` (a full Sanctum-gated REST implementation of the same flow) were both fully intact and working — only the session-authed UI wizard was gone. `App\Models\CloudProviderToken` already had full CRUD UI shipped elsewhere (`Security/CloudTokens.jsx`, `Server/CloudProviderToken.jsx`), and the `servers` table already carried `hetzner_server_id`/`hetzner_server_status` for `Server\Show`'s existing "link an existing server to Hetzner" flow.

**Extracted `App\Actions\Server\CreateHetznerServer`** (this codebase's established `lorisleiva/laravel-actions` pattern) out of `Api\HetznerController::createServer()`'s previously-inlined ~90 lines of SSH-key-upload-or-reuse (matched by MD5 fingerprint against Hetzner's existing keys), param-building, Hetzner-API-create, and local `Server::create()`-with-proxy-defaults logic. `Api\HetznerController` was refactored to call the Action instead of inlining it — behavior-preserving, and a net coverage improvement, since that controller had zero test coverage before this pass.

**New `App\Http\Controllers\ServerCreateHetznerController`** (session-authed, mirrors `ServerShowController`'s existing Hetzner-method shape): `index()` renders the wizard with the team's Hetzner tokens/private keys/saved cloud-init scripts; `data()` takes a `token_id` and returns locations/server-types/images/ssh-keys in one round-trip; `store()` validates, resolves token/private key via `ownedByCurrentTeam()`, optionally saves the cloud-init script as reusable, calls the Action, and redirects to `server.show` on success — the same pattern `ServerController::store()` already uses for the plain-IP creation flow. Routes: `GET /servers/new/hetzner`, `GET /servers/new/hetzner/data`, `POST /servers/new/hetzner`.

**New `resources/js/Pages/Server/New/Hetzner.jsx`** — a 2-step wizard recovered from the deleted Livewire component's own git history (`git show 9d13d92e^:app/Livewire/Server/New/ByHetzner.php`) for field/filter fidelity: Step 1 picks or adds a Hetzner token; Step 2 is Location → Server Type (filtered to types available in that location, with a CPU-vendor label derived from the type-name prefix) → Image (filtered to the server type's architecture) → Server Name (pre-filled via `generate_random_name()`) → Private Key (existing list, or `PrivateKeyCreateModal.jsx` reused unchanged for inline creation) → optional extra Hetzner SSH keys → IPv4/IPv6 toggles (default both on) → optional Cloud-Init script, with a "save as reusable script" checkbox that reuses `App\Models\CloudInitScript` unchanged. Every field got a real `id`/`name` from the start, rather than needing the kind of follow-up sweep Section 149 required. `AddServerModal.jsx` gained an "Add via Hetzner Cloud →" link that closes the modal and navigates to the new page — reachable everywhere the modal already is (`Server/Index.jsx`, `Dashboard.jsx`, `GlobalSearchModal.jsx`). `Boarding\Index.jsx`'s own "IP-only" scope decision (Phase 76/77, for first-run onboarding specifically) was deliberately left as-is; this restores the general/project-wide creation path, not onboarding's narrower one.

**New `tests/v4/Feature/ServerCreateHetznerControllerTest.php`** (10 tests) covers the Inertia page's props and team-scoping, the `data` endpoint against `Http::fake()`-mocked Hetzner responses, successful server creation (including the cloud-init-script-save path), hostname validation rejection, cross-team token/private-key rejection, and 429 rate-limit surfacing through to a flash error rather than a crash.

Verification: Pint clean, PHPStan clean on all new/changed files (`0 errors` on the full configured-`paths:` run — the 5 pre-existing `routes/web.php` errors surfaced only when explicitly targeting that file outside its normal scope are unrelated and unchanged by this diff), full suite 1006 passed (4,726 assertions, up from 996 pre-phase), `yarn build` clean. No real Hetzner account exists in this dev environment, so verification here is `Http::fake()`-based automated tests only, same limitation as every other external-API-touching flow in this migration — flag to the user for real browser/account QA once merged.

## 151. The `/terminal` WebSocket "reconnect loop" was never a WebSocket bug — a real headless-browser session found the actual cause

`docs/smoketest.md` had carried an open item since Phase 24 (2026-07-10): manual QA saw an endless reconnect loop on `/terminal` (code 1006 after a successful auth handshake), attributed at the time to "no genuinely reachable SSH target in this dev environment." Phase 141's follow-up disproved half of that — `Server::find(0)` (the dev seeder's `coolify-testing-host` row) is confirmed SSH-reachable — but left the actual root cause itself blocked, on the stated assumption that terminal WebSocket auth is cookie-based and therefore "not scriptable" without a real browser session.

That assumption turned out to be wrong too. `pestphp/pest-plugin-browser` and `playwright` are already project dependencies (`composer.json`/`package.json`), meaning a genuine, real, automated headless-Chromium session was available the whole time — it had just never been attempted. Getting one running took real infrastructure work, not just `php artisan test`: Pest's browser plugin needs PHP and Node co-located in the same process, but this project's `coolify` container has PHP with no Node, and `coolify-vite` has Node but is Alpine-based (musl libc), which can't run Playwright's downloaded Chromium (glibc-only, no official Alpine support). The fix was a throwaway `mcr.microsoft.com/playwright:v1.61.1-noble` container (version-matched to the project's own `playwright` npm dependency) attached to the `coolify` Docker network via `docker run --network coolify`, driving a real login → `/terminal` → select server → Connect flow against the actual dev stack, with Playwright's native `page.on('websocket')` instrumentation capturing the real WebSocket lifecycle (frames, close events) independent of the app's own console logging.

Two infrastructure wrinkles along the way, both worth recording since they'll recur for any future from-scratch browser-test container: (1) the app always emits Vite dev-server asset URLs as literal `localhost:5173` (`VITE_HOST=localhost` in `docker-compose.dev.yml`), which doesn't resolve from an unrelated container — fixed with a Playwright `context.route()` network-layer redirect to `host.docker.internal:5173`, the actual host-mapped port reachable from any container. (2) Vite's own `server.cors.origin` allowlist (`vite.config.js`) only accepts `localhost`/`127.0.0.1`/`[::1]`, rejecting the `host.docker.internal` page origin — temporarily widened for the diagnostic run, then reverted immediately after (not a permanent product change; `git diff vite.config.js` confirms it round-tripped to a no-op).

**Finding: the original 1006/reconnect-loop theory doesn't reproduce at all.** The WebSocket connects and authenticates cleanly against the real dev target every time. The actual, 100%-reproducible bug was one line into `docker/coolify-realtime/terminal-server.js`'s `handleCommand()`:

```js
const commandString = command[0].split('\n').join(' ');
```

`command` here is already the full plain-string SSH invocation (`SshMultiplexingHelper::generateSshCommand()`'s output, round-tripped through `JSON.stringify({ command })` client-side and `JSON.parse()` server-side) — not an array. `command[0]` silently took its *first character* (`"s"`, from `"ssh ..."`) instead of the whole string, before it ever reached `extractSshArgs()`. Every real terminal connection therefore failed at the very next step with `"Invalid SSH command: No target host found"` — reproduced with the exact live-captured command text run through the real parser both via `handleCommand()`'s buggy path (fails) and directly against `extractSshArgs()`/`extractTargetHost()` in isolation (succeeds), conclusively isolating the bug to that one line. `git log -p` confirms `command[0]` has been there since this file's very first commit in the repo's history — predating the Livewire→React migration entirely, meaning the browser-based terminal feature has likely never actually worked in this fork, for any server, the whole time it's existed.

This also explains why the existing `terminal-utils.test.js` unit tests (`extractSshArgs`/`extractTargetHost`, both well-covered with realistic quoted-argument cases) never caught it: they call those pure functions directly with an already-correct full string, never through the actual `command[0]`-truncating integration point in `terminal-server.js`.

**Fix**: changed `command[0]` to `command`. Extracted the flatten-then-parse sequence (`.split('\n').join(' ')` + the four `extract*()` calls) into a new `parseCommandMessage(rawCommand)` in `terminal-utils.js`, so the exact code path `handleCommand()` runs is now unit-testable in isolation rather than only reachable through a live WebSocket session. Two new tests in `terminal-utils.test.js`: one asserting `parseCommandMessage()` correctly extracts the target host from the real captured command string; one explicitly asserting the fix's output differs from what `parseCommandMessage(rawCommand[0])` would produce (i.e., a test that would fail again if this exact regression reappeared, not just a happy-path check).

Verified live, end to end, after the fix, via the same throwaway Playwright container: `pty-ready` received, followed by a real shell prompt (`root@<container-id>:~#`) streamed back over the socket — the terminal genuinely works now. `node --test docker/coolify-realtime/terminal-utils.test.js resources/js/terminal.test.js` (10 tests, this project's Node-native test runner — no Vitest/Jest config exists), full Pest suite (1006 passed, unchanged — no PHP touched), Pint clean (no PHP touched), `yarn build` clean. `docs/smoketest.md`'s corresponding checklist item updated from an open manual-QA note to resolved, since this is now covered by an automated regression test rather than needing a human to re-click through it.

## 152. Two more waves of Alpine remnants — toast notifications, then the password toggle/info tooltip/2FA challenge

Manual smoke-test QA (Section 6's checklist, Cross-cutting item 7) found toast notifications completely non-functional across the entire React app: `components/toast.blade.php` — the only place `window.toast()` was ever actually *defined* — is built entirely on Alpine.js (`x-data`/`x-init`/`x-teleport`/`@toast-show.window`), and Section 148 above already predicted this dependency (see its note on `copyToClipboard()`) without following through on it. Alpine was fully removed from `package.json` finishing this migration, and `app-inertia.blade.php` never included the component in the first place, so every `window.toast(...)` call site in the React codebase (`AppLayout.jsx`'s flash handler, `ServerNavbar.jsx`, `TerminalWindow.jsx`, `Server/Proxy.jsx`) had been silently no-op'ing since Section 148 shipped — no crash, no console error, nothing to catch it except actually watching for the toast in a real browser. Fixed with a from-scratch React port, `resources/js/Components/Toast.jsx` (`window.toast(title, options)`, same call signature, deliberately narrower than the ~500-line Alpine original — only `title`/`type`/`description`, the only fields any real call site used). Verified live via the throwaway-Playwright-container technique: real login, real form save, `typeof window.toast` confirmed `"function"`, the actual rendered toast text read back matching the backend's real flash message.

That fix's own Tailwind-lint-warning cleanup prompted the question "wait, didn't we remove Alpine completely?" instead of just patching the warning — and a full-repo sweep (`grep -rlE 'x-data=|x-init=|x-show=|x-cloak|x-teleport|x-text=|x-html=|x-transition|x-effect|x-model=|@click=|@keydown|Alpine\.'`) found the npm package really was gone, but three more real, user-facing Blade files still had live Alpine directive markup with zero runtime behind it: `components/forms/input.blade.php` (the password show/hide eye-icon toggle — login, register, reset-password, confirm-password all use it), `components/helper.blade.php` (the click-to-open info-tooltip, currently unreferenced by any live page), and `auth/two-factor-challenge.blade.php` (the 2FA code screen's digit-auto-advance inputs, paste-to-fill, and recovery-code toggle). Rewrote all three in plain vanilla JS delegated via `data-*` attributes in `app.js` (the shared entrypoint for these plain-Blade pages), 1:1 ports of the original Alpine methods' actual traced behavior. Also deleted `toast.blade.php` itself (now fully superseded), its `<x-toast />` reference and `copyToClipboard()`/`[x-cloak]` remnants in `base.blade.php`, and — once those left `window.sanitizeHTML()` with zero callers — that DOMPurify wrapper, its `<script src=".../purify.min.js">` tag, and the vendored `public/js/purify.min.js` asset.

Password toggle verified live against the real `/login` page via the same throwaway-Playwright technique (`type` genuinely flips `password ↔ text`, correct icon swap, zero console errors). 2FA logic verified against a structurally-identical static harness loading the real, unmodified `app.js` — a live authenticated session wasn't practical without creating a 2FA-enabled test user (Laravel Boost's tinker guidance says not to without approval), so the harness exercises the actual shipped JS against the actual shipped markup instead. `helper.blade.php` has no live call site today, so it's verified by code review + a full `php artisan view:cache` recompile only — flagged rather than silently assumed correct.

Both waves are a reminder that "the npm package is gone" and "every place that used it still works" are different claims — Section 148's own audit predicted exactly this gap for `toast.blade.php` and didn't chase it down at the time. See `todo.md`'s "Cleanup opportunities" section for the full evidence trail; tracked as Scrum board issues #30 and #31.
