...
4. `node_modules` and `vendor` don't need pre-installing on the host: `coolify-vite`'s container command already runs `yarn install` on startup, and the `coolify` container's own entrypoint runs `composer install` automatically against the bind mount.
