...
| `coolify-https-proxy` | **Opt-in, not started by `spin up`/the two default compose files** — added 2026-07-30 as a friendly local dev URL, `https://coolify-full.localhost:8443`. `*.localhost` resolves to loopback natively in browsers and Windows (no hosts-file edit needed); port 8443 was chosen because `coolify-proxy` (the real Traefik proxy this repo uses to test Coolify's own routing features, not a spare port) already owns 80 and 8080. Start with `./docker/https-proxy/generate-cert.sh` once (self-signed, gitignored — not committed; also adds `VITE_HOST` as a cert SAN so Vite's own dev-server assets, served over HTTPS too once the cert exists, don't trigger a second CN-mismatch warning), then `docker compose -f docker-compose.yml -f docker-compose.dev.yml -f docker-compose.https.yml up -d https-proxy`. Also terminates TLS on port 6443 for Soketi's websocket connection (real-time UI updates) — `coolify-realtime`'s real port, 6001, only speaks plain `ws://`, which browsers refuse to connect to from an HTTPS page (mixed content, same rule as an HTTP `<script>`); `getRealtime()`/`HandleInertiaRequests` switch to 6443 + `forceTLS: true` automatically when the request is secure. **Trade-off worth knowing**: once the cert exists, Vite serves *all* its assets over HTTPS for every access path, including plain `http://localhost:8000` — a one-time browser cert-warning click-through lands on the default workflow too, not just the new URL (a second, separate click-through for the 6443 websocket port too, first time it's used). See `docker-compose.https.yml`, `vite.config.js`, and `docker/https-proxy/nginx.conf`. |
