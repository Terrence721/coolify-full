...
**`dev-up.sh` now also runs automatically when this workspace opens in VS Code**, via `.vscode/tasks.json`'s `runOn: folderOpen` (that file is gitignored, machine-local, same as `settings.json` — not something every contributor needs). The gap this closes: several containers (`coolify-realtime`, `coolify-testing-host`, `https-proxy`) were found sitting `Exited (127)` for 2 days after a reboot nobody ran `dev-up.sh` after, which also showed up as real symptoms in the app itself — a Coolify-managed server going "Not reachable" in the UI because its Docker-in-Docker host (`coolify-testing-host`) was one of the stuck containers. `./scripts/dev-down.sh` is the counterpart for shutting the stack down (bound to VS Code's "Stop Dev Stack" task — Terminal → Run Task — since VS Code has no reliable "on workspace close" hook to automate this half); it's a plain `compose stop`, not `compose down`, so containers aren't recreated on the next start and the bind-mount race above doesn't get a chance to reoccur.

**Also fixed the same investigation**: `DB_HOST` was `host.docker.internal`, hairpinning every database connection out through the Windows/WSL2 host-gateway NAT instead of the `coolify`/`coolify-db` containers' shared Docker network — intermittently failing with `SQLSTATE[08006]... server closed the connection unexpectedly` on long-running setups. Changed to the `coolify-db` compose service name (both in `.env` and the `.env.development.example` template, matching `MAIL_HOST`'s already-correct pattern) — internal Docker DNS instead of the flaky host-gateway route.

**Fixed 2026-09-14, found by a full-codebase security audit**: `postgres` and `redis` in `docker-compose.dev.yml` published their ports on `0.0.0.0` (any network interface) instead of `127.0.0.1`, and Postgres additionally set `POSTGRES_HOST_AUTH_METHOD: "trust"` — together, any machine on the same LAN could connect to the dev database with zero credentials. Neither binding affects the app itself, which always connects over the Docker-internal network by service name (`DB_HOST=coolify-db`), never through the published host port. Both now bind to `127.0.0.1` only, and the `trust` override was removed so a fresh Postgres volume defaults to real password auth. **If you already have an existing `dev_postgres_data` volume** (i.e. you're pulling this fix onto a dev environment that was already running), the compose change alone won't retroactively fix it — `pg_hba.conf` is written once at first volume initialization, not regenerated on restart. Apply it live instead: `docker exec coolify-db sh -c "sed -i 's/trust/scram-sha-256/g' /var/lib/postgresql/data/pg_hba.conf" && docker exec coolify-db psql -U coolify -d coolify -c "SELECT pg_reload_conf();"` — no restart needed, and the app's own connections are unaffected since it already sends the configured password on every connect.
