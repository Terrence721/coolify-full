...
- [ ] **Follow-up, planned 2026-07-20 — configure real LAN/intranet access, not just confirm the known gap above.** Tracked separately (see the Scrum board sub-issue linked at the top of this file) since it's actual configuration + validation work, not a read-through check. Plan: (1) find the host machine's real LAN IP (Windows `ipconfig`, the IPv4 address of the active adapter — not WSL2's own internal IP); (2) set `VITE_HOST=<that-ip>` in `.env`; (3) restart `coolify-vite` to pick up the new value; (4) confirm Windows Firewall allows inbound LAN traffic on ports 8000 and 5173; (5) from another machine on the same network, visit `http://<that-ip>:8000` and confirm the app loads **fully working this time** — assets included, not just the bare HTML the check above already confirmed. This is a real, deliberate test of intranet reachability, closer to how a genuine self-hosted deployment would actually be accessed by other machines on the network.
