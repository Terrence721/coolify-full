...
**A red badge/counter in DevTools' own toolbar (next to the settings gear) is not a page error.** Confirmed 2026-07-20 in Microsoft Edge DevTools: a persistent red "2" badge showed up across multiple sessions even when the Console had zero red error lines and the Issues panel explicitly said "No Issues." That badge is DevTools-level (browser feature notifications/tips), unrelated to this webpage — don't chase it. The actual signal to watch is the Console log itself (red error lines) and the Issues panel's own count, not that toolbar badge.
