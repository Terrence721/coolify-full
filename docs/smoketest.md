...
- [ ] Welcome screen renders; "Let's go!" advances to the platform-overview explanation (or, on a cloud-flagged instance, skips straight to the private-key step — note if you're testing on one).
- [ ] "This Machine" (localhost) tile immediately kicks off validation against the id-0 default server and lands on the Validate Server step with no intermediate form.
- [ ] "Remote Server" tile → SSH key step: selecting an existing key advances directly to server details; "Use Existing Key"/"Generate New Key" both open the shared private-key modal (paste or generate ED25519/RSA), and creating a key auto-advances to server details with the new key selected.
- [ ] Server details form: submitting a duplicate IP (one already in the team) shows the inline error without creating a second row; a genuinely new IP creates the server and jumps straight into validation.
- [ ] Validate Server step against a real, reachable server: confirm the live install log actually streams real output (not stuck on "Waiting for the process to start...") during a prerequisites/Docker install, and that it retries automatically (up to 3 attempts) before surfacing a manual-install error.
- [ ] Validate Server step against an unreachable server (wrong IP/port/key): confirm the "Server is not reachable" error renders with a Retry button, not a blank page.
...
- [ ] `/server/{uuid}/terminal` (Command tab, `ExecuteContainerCommandController::server()`) — connect a shell against the server itself (not a container); confirm it behaves the same as the generic `/terminal` page's checks above (real prompt, streaming output, reconnect-on-visibility-change).
- [ ] `/server/{uuid}/proxy/logs` — confirm real proxy container log output streams/loads (not a blank page); download works.
- [ ] `/server/{uuid}/sentinel/logs` — confirm real Sentinel container log output streams/loads; download works.
